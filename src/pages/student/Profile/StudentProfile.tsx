...
const StudentProfile: React.FC = () => {
  ...
  const [studyFocus, setStudyFocus] = useState(profileData.studyFocus)
  const [avatarBase64, setAvatarBase64] = useState<string | null>(profileData.avatarBase64)
  const [saved, setSaved] = useState(false)
  const fileRef = useRef<HTMLInputElement>(null)

  // Convert selected image file → base64 and preview immediately
  ...
  const handleSave = () => {
    if (!user) return
    storage.setProfile(user.id, { bio, studyFocus, avatarBase64 })
    setEditing(false)
    setSaved(true)
    ...
    <div className="min-w-0 space-y-5 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/10 bg-slate-950/85 p-5 shadow-2xl shadow-slate-950/20 sm:rounded-3xl sm:p-8">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold text-white sm:text-3xl">{user?.name}</h2>
          <span className="mt-2 inline-block rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">
            Active learner
          </span>
        </div>
        {!editing && (
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-xl shadow-slate-950/20 sm:rounded-3xl sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-full border-2 border-emerald-400/30 bg-slate-800">
              {avatarBase64 ? (
          ...
          <div className="flex-1 space-y-4">
            ...
            <div>
              <label className="block text-sm text-slate-400">Study focus</label>
              {editing ? (
                <input
                  type="text"
                  value={studyFocus}
                ...
                <p className="mt-1 text-slate-300">{studyFocus || '—'}</p>
              )}
            </div>
      ...
      <div className="grid gap-4 sm:gap-5 md:grid-cols-2 xl:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-xl shadow-slate-950/15 sm:rounded-3xl sm:p-6">
          ...
          <p className="mt-2 text-lg font-semibold text-white">Mentoro Pro</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-xl shadow-slate-950/15 sm:rounded-3xl sm:p-6">
          <h3 className="text-sm text-slate-400">Account role</h3>
          <p className="mt-2 text-lg font-semibold text-white capitalize">{user?.role}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-xl shadow-slate-950/15 sm:rounded-3xl sm:p-6">
          <h3 className="text-sm text-slate-400">Member ID</h3>
