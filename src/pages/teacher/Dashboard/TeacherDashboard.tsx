...
const TeacherDashboard: React.FC = () => {
  ...
  const stats = [
    ...
    { label: 'Published',      value: published,             icon: '✅', color: 'var(--m-success)' },
    { label: 'Videos',         value: totalVideos,           icon: '🎬', color: 'var(--m-info)' },
    { label: 'Documents',      value: totalDocs,             icon: '📄', color: 'var(--m-warning)' },
  ]
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', minWidth: 0 }}>
      ...
      {error && (
        <div style={{ borderRadius: 'var(--m-r-md)', border: '1px solid var(--m-danger)', background: 'var(--m-danger-soft)', padding: '0.875rem 1rem', fontSize: '0.875rem', color: 'var(--m-danger-text)' }}>
          {error}
        </div>
      )}
      {loading && (
